...
export class html_content {
	...
	static clockScripts(clock, server) {
		return `
...
<script type="text/javascript">
	...
	function selectSequence(select) {
		postToURL("http://${clock.prefs.name}.local/layout",{sequence:select.value});
	}
	function selectWebSequence(select) {
		postToURL("http://${clock.prefs.name}.local/layout",{webSeq:select.value});
	}
...
*/
	...
	static selection(name, items, selected, indexed=1, params="", sclass="select-css") {
		let x = `<select class="${sclass}" name="${name}" ${params}>`;
...
if (check_update("${clock.getbuildstring().trim()}"))
	...
	static clockConfigSection(clock) {
		let filenames = clock.fileMgr.getFilenameList();
		let webnames = clock.fileMgr.getWebnameList();
		webnames.sequences.push("<select>");
		let body = `<h2>Config</h2>
Device: 
${html_content.selection("device", filenames.dev, clock.prefs.device, 0)}
<div><p>
Sequence:
${html_content.selection("sequence", filenames.seq, clock.prefs.sequence, 0, 'id="sequence" onchange="selectSequence(this)"')}
<div><p>
Web:
${html_content.selection("webseq", webnames.sequences, "<select>", 0, 'id="webseq" onchange="selectWebSequence(this)"')}
<div><p>

Put a JSON object with a <b>device</b> and/or <b>sequence</b><p>
<form action="/layout" method="post">
<div><p>
<textarea id="device" name="device" rows="40" cols="180">
{
    "device": ${JSON.stringify(clock.device)},\n
    "sequence": ${JSON.stringify(clock.sequence)} }
</textarea>
<div><button class="button pressButton">Set Device config</button></div></form>`;
		return body;
	}
...
${html_content.slider("speed", clock.prefs.speed, 1, 500)} %
</div><p>
